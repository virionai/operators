...
The app uses a quiet operational trust surface: black background, low-contrast grid, thin ledger borders, blue/sage/amber/rose status semantics, and isolated Gemma gradient energy on the assistant dock and modal.
...
- Workspace: left intelligence rail, spatial canvas, floating document viewer, Gemma modal.
...
- Start Here opens an editable declaration surface preloaded with local Gemma/Ollama, Capsule protocol, artifact staging, persistence, and cryptographic hook capabilities.
...
- Clicking files, selected text, or workspace modules queues compact visual context chips in Gemma instead of auto-running inference.
...
- Resident overlay Gemma panel with Q&A, queued context chips, queued follow-up task cards, communication-plan cards, generated component frames, and cancelable inference.
- Decision Gates are schema-populated operational checkpoints. Gemma returns them as evidence-linked JSON, and the rail renders them as human-reviewable gates with source attribution.
- Gemma-detected concerns become rose-coded workspace items and matching Gemma queue chips so the operator can correlate chat output with canvas state. Generated concern notes render preview-only; graph-queued concerns render as graph/topology modules.
...
- No modal-heavy interaction except Gemma and sealing.
- Gemma is a resident analysis layer, not a chatbot page.
