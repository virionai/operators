...
const workspaceBlock = sliceBetween(css, "/* Command workspace formatting reset */", "/* End Command workspace formatting reset */");
const legacyWorkspaceBlock = sliceBetween(css, ".command-workspace-view .agent-workspace-render {", "}");
...
const checks = [
  ...
  [
    ...
    commandPanel.includes("renderedWorkspaceModules.length") &&
      !commandPanel.includes("canvasModules.length} rendered surface"),
  ],
  [
    "agent workspace no longer stretches across many grid rows",
    workspaceBlock.includes(".command-workspace-view .agent-workspace-render") &&
  ...
  [
    ...
    workspaceBlock.includes(".command-workspace-view .agent-render-card") &&
      workspaceBlock.includes("min-height: 0") &&
      workspaceBlock.includes(".command-workspace-view .agent-render-card .markdown-render") &&
      workspaceBlock.includes("max-height: 240px"),
  ],
  [
    "message grid aligns workspace products at the top",
    workspaceBlock.includes(".command-workspace-view .message-stack") &&
